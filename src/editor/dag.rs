...
mod tests {
    ...
    mod reg {
        ...
        /// this is a regression test for pull request #100.
        #[test]
        fn pr_100_prefix_replace_crash() {
            let arena: Arena<Json> = Arena::new();
            let root = add_value_to_arena(json!([true, false, false]), &arena);
            let mut dag = Dag::new(&arena, root, Path::from_vec(vec![0]));

            // Replace the [true, false, false] to [false, false, false, false]
            assert_eq!(
                Err(EditErr::ReplaceError {
                    current_child_index: 0,
                    last_child_index: 2,
                    node_count: 4
                }),
                dag.replace_cursor(4, Insertable::CountedNode(1, 'f'))
            );
        }
    }
}
