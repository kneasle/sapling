...
pub mod normal_mode{
    ...
    impl KeyStroke {
        ...
        pub fn summary_string(&self) -> &'static str {
            match self {
                ...
                KeyStroke::MoveCursor(super::Direction::Down) => "move to first child",
                KeyStroke::MoveCursor(super::Direction::Up) => "move to parent",
                KeyStroke::MoveCursor(super::Direction::Prev) => "move to previous sibling",
                KeyStroke::MoveCursor(super::Direction::Next) => "move to next sibling",
                KeyStroke::Undo => "undo",
                KeyStroke::Redo => "redo",
            }
        }
    ...
    pub fn default_keymap() -> KeyMap {
        hmap::hmap! {
            ...
            'r' => KeyStroke::Replace,
            'x' => KeyStroke::Delete,
            'c' => KeyStroke::MoveCursor(super::Direction::Down),
            'p' => KeyStroke::MoveCursor(super::Direction::Up),
            'h' => KeyStroke::MoveCursor(super::Direction::Prev),
            'j' => KeyStroke::MoveCursor(super::Direction::Next),
            'k' => KeyStroke::MoveCursor(super::Direction::Prev),
            'l' => KeyStroke::MoveCursor(super::Direction::Next),
            'u' => KeyStroke::Undo,
            'R' => KeyStroke::Redo
        }
    }
    ...
    impl Action {
        ...
        pub fn description(&self) -> String {
            match self {
                ...
                Action::MoveCursor(super::Direction::Down) => "move to first child".to_string(),
                Action::MoveCursor(super::Direction::Up) => "move to parent".to_string(),
                Action::MoveCursor(super::Direction::Prev) => "move to previous sibling".to_string(),
                Action::MoveCursor(super::Direction::Next) => "move to next sibling".to_string(),
...
impl<'arena, Node: Ast<'arena> + 'arena> Editor<'arena, Node> {
    ...
    pub fn new(
        ...
        Editor {
            ...
            command: String::new(),
            keymap,
            command_log: normal_mode::command_log::CommandLog::new(10),
        }
    }
    ...
    fn execute_action(&mut self, action: normal_mode::Action) -> (bool, EditResult) {
        let mut should_quit = false;
        // Respond to the action
        let result = match action {
            // Undefined command
            normal_mode::Action::Undefined => Err(EditErr::Invalid(self.command.clone())),
            // History commands
            normal_mode::Action::Undo => self.tree.undo(),
            normal_mode::Action::Redo => self.tree.redo(),
            // Move command
            normal_mode::Action::MoveCursor(direction) => self.tree.move_cursor(direction),
            // Edit commands
            normal_mode::Action::Replace(c) => self.tree.replace_cursor(c),
            normal_mode::Action::InsertChild(c) => self.tree.insert_child(c),
            normal_mode::Action::InsertBefore(c) => self.tree.insert_next_to_cursor(c, Side::Prev),
            normal_mode::Action::InsertAfter(c) => self.tree.insert_next_to_cursor(c, Side::Next),
            normal_mode::Action::Delete => self.tree.delete_cursor(),
            // Quit Sapling
            normal_mode::Action::Quit => {
                should_quit = true;
                Ok(EditSuccess::Quit)
            }
        };
    ...
    fn consume_command_char(&mut self, c: char) -> (bool, Option<EditResult>) {
        ...
        // complete
        match normal_mode::parse_command(&self.keymap, &self.command) {
            Some(action) => {
...
#[cfg(test)]
mod tests {
    use super::{normal_mode::parse_command, normal_mode::Action};
